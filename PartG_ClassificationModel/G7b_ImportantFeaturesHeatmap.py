...
# %% [markdown]
# # Profile heatmaps for the most important features
...
feature_importances_csv = sys.argv[6]


# Extract the "important" motifs to plot
motifs_importances = pd.read_csv(feature_importances_csv)
important_motifs = motifs_importances.iloc[0:50,:]
bio_selected_motifs = important_motifs["motif"].to_list()
...
def plot_average_profile_heatmap(data, rank, selected_motifs, png_name):
    ...
    sns.heatmap(profile_data, cmap="Blues", yticklabels=taxon_labels, xticklabels=selected_motifs)   # "crest_r"
    plt.xlabel("Motifs")
...
for taxonomy, average_profile in (["species", average_profile_per_species],
        ...
        selected_motifs = bio_selected_motifs
        png_name = f"{output_folder}/ImportantMotifs_AverageProfilePer_{taxonomy}.png"
